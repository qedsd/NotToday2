...
namespace NotToday.Helpers
{
    public class WindowHelper
    {
        ...
        {
            var hWnd = GetWindowHandle(window);
            var styleCurrentWindowStandard = Win32.GetWindowLongPtr(hWnd, -16);
            var styleNewWindowStandard = styleCurrentWindowStandard & ~(0x00040000 | 0xc00000 | 0x80000 | 0x10000 | 0x20000);
            if (Win32.SetWindowLongPtr(hWnd, -16, (IntPtr)styleNewWindowStandard) == IntPtr.Zero)
            {
                //fail
            }
        }
        ...
        public static void SetForegroundWindow2(IntPtr targetHandle)
        {
            ...
            Log.Debug($"激活窗口 {targetHandle}({curForegroundWindow})");
        ...
        public static void SetForegroundWindow3(IntPtr targetHandle)
        {
            ...
            {
                Log.Debug($"AttachThreadInput失败：{dwForeID}->{dwCurID}");
                return;
            }
            int tryCount = 0;
            while (tryCount++ < 3)
            {
                if (Win32.SetForegroundWindow(targetHandle))
                {
                    ...
                    {
                        Log.Debug($"SetForegroundWindow成功但未生效（{tryCount}）");
                        Thread.Sleep(100);
                    }
                    else
                    {
                        ...
                        while (tryCount < 3)
                        {
                            if (Win32.BringWindowToTop(targetHandle))
                            {
                                ...
                                {
                                    Log.Debug($"成功激活窗口{targetHandle}");
                                    break;
                                }
                                else
                                {
                                    Log.Debug($"解除AttachThreadInput失败");
                                }
                            }
                            else
                            {
                                Log.Debug($"BringWindowToTop失败（{tryCount}）");
                            }
                        }
                        break;
                    }
                ...
                {
                    Log.Debug($"SetForegroundWindow失败（{tryCount}）");
                }
            }
        }
        ...
        public static void SetForegroundWindow4(IntPtr targetHandle)
        {
            ...
            {
                Log.Debug($"AttachThreadInput失败：{dwCurID}->{dwForeID}");
                return;
            }
            int tryCount = 0;
            while (tryCount++ < 3)
